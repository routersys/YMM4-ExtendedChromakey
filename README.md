...
- **最高精度の色差計算 (CIEDE2000)**: 従来の計算では難しい微妙な色の差を正確に判定する「CIEDE2000」を搭載。最高の精度でキーイングや色補正が可能です。（高品質・高負荷）
...
- **強化された色補正機能**: クロマキーで残った不要な色かぶりを、前景全体から自然な質感（輝度）を保ったまま他の色に変換して目立たなくすることができます。
...
| | 色空間(カスタム用) | `主要キーイング色`が`カスタム`の時に、色距離の計算に使用する色空間を選択します。最高の精度を誇る「CIEDE2000」も選択可能です。 |
...
| **色補正** | 補正対象色 | 補正したい残存色を指定します。（例：残った緑色） |
| | 補正後の色 | 残存色をこの色に変換します。（例：グレー） |
| | 残存色補正 | クロマキーで残った背景色を他の色に変換する強度を指定します。 |
